...
if (function_exists('eccube_log_init') === false) {
    ...
    function eccube_log_init($app)
    {
        ...
        $app['eccube.monolog.factory'] = $app->protect(function ($config) use ($app) {
            $logger = new $app['monolog.logger.class']($config['name']);

            $filename = $app['config']['root_dir']."/app/log/{$config['filename']}.log";
            $RotateHandler = new \Monolog\Handler\RotatingFileHandler($filename, $app['config']['log']['max_files'], \Monolog\Logger::INFO);
            $RotateHandler->setFilenameFormat(
                $config['filename'].'_{date}',
                'Y-m-d'
            );
